...
if __name__ == '__main__':
	...
	if soc.gethostname()[:6] == "yzpc":
		 main_path = '/home/yz/human_anticipation_data/CAD_120/features_cad120_ground_truth_segmentation'

	else:
		print 'can not find host'
	...
	num_sub_activities = int(np.max(Y_tr_human) - np.min(Y_tr_human) + 1)   #np.max 求指定维度的最大值
	...
	trX = {}
	trY = {}
	trX['H:H'] = np.concatenate((X_tr_human_shared,X_tr_human_disjoint),axis=2)	   #np.concatenate 数组拼接函数
